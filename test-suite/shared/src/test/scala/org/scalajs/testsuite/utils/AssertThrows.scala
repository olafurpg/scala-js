...
object AssertThrows {
  ...
  private def expectThrowsBackport[T <: Throwable](
    ...
    try {
      runnable.run()
      val message =
        s"expected ${expectedThrowable.getSimpleName} to be thrown," + " but nothing was thrown"
      ...
      case actualThrown: Throwable =>
        if (expectedThrowable.isInstance(actualThrown)) {
          ...
          val mismatchMessage =
            "unexpected exception type thrown;" + expectedThrowable.getSimpleName + " " + actualThrown.getClass.getSimpleName
  ...
  def assertThrows[T <: Throwable, U](
    ...
    assertThrowsBackport(
        expectedThrowable, throwingRunnable(code.asInstanceOf[Unit]))
