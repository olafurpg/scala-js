...
class UTF8Test extends BaseCharsetTest(Charset.forName("UTF-8")) {
  ...
  def decode(): Unit = {
    ...
    if (!executingInJVM) {
      testDecode(bb"${(0xc0 to 0xf4).flatMap(c => Seq(c, 32))}")((0xc0 to 0xf4)
        .flatMap(i => OutSeq(Malformed(1), cb" ")): _ *)
    }
    ...
    testDecode(bb"c2  e0  e0 a0  f0  f0 90  f0 90 80")(Seq(1, 1, 2, 1, 2, 3)
      .map(Malformed(_)): _ *)
    // and with normal sequences interspersed
    testDecode(bb"c2 41 e0 41 e0 a0 41 f0 41 f0 90 41 f0 90 80 41")(Seq(
        1, 1, 2, 1, 2, 3).flatMap(l =>
  ...
  def isLegalReplacement(): Unit = {
    ...
    assertTrue(encoder.isLegalReplacement(Array(
        0xe0.toByte, 0xa0.toByte, 0x80.toByte)))
    assertTrue(encoder.isLegalReplacement(Array(
        0xef.toByte, 0xbf.toByte, 0xbf.toByte)))

    assertTrue(encoder.isLegalReplacement(Array(
        0xf0.toByte, 0x90.toByte, 0x80.toByte, 0x80.toByte)))
    assertTrue(encoder.isLegalReplacement(Array(
        0xf4.toByte, 0x8f.toByte, 0xbf.toByte, 0xbf.toByte)))
