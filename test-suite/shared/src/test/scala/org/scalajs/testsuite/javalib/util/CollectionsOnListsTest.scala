...
object CollectionsOnListTest extends CollectionsTestBase {
  ...
  def sort_with_comparator(factory: ListFactory): Unit = {
    if (factory.sortableUsingCollections) {
      ...
      test_sort_with_comparator[Int](
          factory, _.toInt, (x, y) => x.compareTo(y))
      test_sort_with_comparator[Long](
          factory, _.toLong, (x, y) => x.compareTo(y))
      test_sort_with_comparator[Double](
  ...
  private def test_sort_on_comparables[T <: AnyRef with Comparable[T]](
    ...
    def testIfSorted(rangeValues: Boolean): Unit = {
      for (i <- range.init) assertTrue(list(i).compareTo(list(i + 1)) <= 0)
      if (absoluteOrder && rangeValues) {
        for (i <- range) assertEquals(0, list(i).compareTo(toElem(i)))
      }
    }
  ...
  private def test_sort_with_comparator[T](
      factory: ListFactory,
      toElem: Int => T,
      cmpFun: (T, T) => Int,
      absoluteOrder: Boolean = true): Unit = {
    ...
    def testIfSorted(rangeValues: Boolean): Unit = {
      for (i <- range.init) assertTrue(cmpFun(list(i), list(i + 1)) <= 0)
      if (absoluteOrder && rangeValues) {
        for (i <- range) assertEquals(0, cmpFun(list(i), toElem(i)))
      }
    }
...
trait CollectionsOnListTest extends CollectionsOnCollectionsTest {
  ...
  def swap(): Unit = {
    ...
    def test[T](toElem: Int => T): Unit = {
      ...
      for (i <- range.drop(2)) assertTrue(list.get(i) == toElem(i))

      ju.Collections.swap(list, 0, range.last)
      assertTrue(list.get(0) == toElem(range.last))
      assertTrue(list.get(1) == toElem(0))
      for (i <- range.drop(2).init) assertTrue(list.get(i) == toElem(i))
  ...
  def fill(): Unit = {
    ...
    def test[T](toElem: Int => T): Unit = {
      ...
      for (i <- range) assertTrue(list.get(i) == toElem(0))

      ju.Collections.fill(list, toElem(42))
      for (i <- range) assertTrue(list.get(i) == toElem(42))
    }
  ...
  def copy(): Unit = {
    ...
    def test[T](toElem: Int => T): Unit = {
      ...
      for (i <- range) assertTrue(dest(i) == toElem(i))
      ...
      range.foreach(i => dest.add(toElem(-i)))
      ju.Collections.copy(dest, source)
      for (i <- range.take(range.size / 2)) assertTrue(dest(i) == toElem(i))
      for (i <- range.drop(range.size / 2)) assertTrue(dest(i) == toElem(-i))
  ...
  def rotate(): Unit = {
    ...
    def test[E](toElem: Int => E): Unit = {
      ...
      for (i <- range) assertTrue(list(i) == toElem(i))

      ju.Collections.rotate(list, list.size)
      for (i <- range) assertTrue(list(i) == toElem(i))

      ju.Collections.rotate(list, 1)
      for (i <- range) assertTrue(list(i) == toElem(modulo(i - 1, range.size)))

      ju.Collections.rotate(list, 1)
      for (i <- range) assertTrue(list(i) == toElem(modulo(i - 2, range.size)))

      ju.Collections.rotate(list, -5)
      for (i <- range) assertTrue(list(i) == toElem(modulo(i + 3, range.size)))

      list.clear()
      list.addAll((0 until 6).map(toElem))
      ju.Collections.rotate(list, 2)
      for (i <- 0 until 6) assertTrue(list(i) == toElem(modulo(i - 2, 6)))
    }
