...
abstract class GenJSCode
  ...
  private [ this] object pos2irPosCache {
    ...
    private [ this] def convert(
        nscSource: SourceFile): ir.Position.SourceFile = {
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    private val translatedAnonFunctions = mutable.Map.empty[
        Symbol,
        /*ctor args:*/ List[js.Tree] =>
        /*instance:*/ js.Tree]
        ...
        for (cd <- fullClassDefs) {
          ...
          val isRawJSImplClass =
            sym.isImplClass && isRawJSType(sym.owner.info
              .decl(sym.name.dropRight(nme.IMPL_CLASS_SUFFIX.length)).tpe)
    ...
    def genClass(cd: ClassDef): js.ClassDef = {
      ...
      if (hasDefaultCtorArgsAndRawJSModule(sym)) {
        reporter.error(
            pos,
            "Implementation restriction: constructors of " + "Scala classes cannot have default parameters " + "if their companion module is JS native.")
      }
      ...
      val hashedDefs = Hashers.hashDefs(generatedMembers ++ exports)
    ...
    def genScalaJSDefinedJSClass(cd: ClassDef): js.ClassDef = {
      ...
      assert(
          ...
          "genScalaJSDefinedJSClass() must be called only for " + s"Scala.js-defined JS classes: $sym")
      assert(sym.superClass != NoSymbol, sym)
      ...
      val generatedMembers = {
        genClassFields(cd) ::: genJSClassConstructor(
            ...
            sym,
            dispatchMethodNames
              .result().distinct) ::: generatedMethods.toList ::: exports
      }

      // Hashed definitions of the class
      val hashedDefs = Hashers.hashDefs(generatedMembers)
    ...
    def genJSClassConstructor(
      ...
      if (hasDefaultCtorArgsAndRawJSModule(classSym)) {
        reporter.error(
            pos,
            "Implementation restriction: constructors of " + "Scala.js-defined JS classes cannot have default parameters " + "if their companion module is JS native.")
        js.EmptyTree
      } else {
    ...
    private class JSConstructorBuilder(root: ConstructorTree) {
      ...
      private def mkOverrideNumsCond(numRef: js.VarRef, numBounds: (Int, Int))(
        ...
        numBounds match {
          ...
          case (lo, hi) =>
            val lhs =
              js.BinaryOp(js.BinaryOp.Num_<=, js.IntLiteral(lo), numRef)
            val rhs =
              js.BinaryOp(js.BinaryOp.Num_<=, numRef, js.IntLiteral(hi))
    ...
    def genMethodWithCurrentLocalNameScope(dd: DefDef): Option[js.MethodDef] = {
      ...
      withScopedVars(
        ...
        val params =
          if (vparamss.isEmpty) Nil
          else vparamss.head map (_.symbol)
        ...
        if (scalaPrimitives.isPrimitive(sym)) {
          ...
          val body =
            if (sym.hasAnnotation(JavaDefaultMethodAnnotation)) {
             ...
             */
              ...
              val implMethodSym = implClassSym.info.member(sym.name).suchThat {
                s =>
                  s.isMethod && s.tpe.params.size == sym.tpe.params.size + 1 &&
                  s.tpe.params.head.tpe =:= sym.owner.toTypeConstructor &&
                  s.tpe.params.tail.zip(sym.tpe.params).forall {
                    case (sParam, symParam) => sParam.tpe =:= symParam.tpe
                  }
              }
              genTraitImplApply(
                  implMethodSym,
          ...
          Some(js.MethodDef(static = false,
                            methodName,
                            jsParams,
          ...
          withScopedVars(
            ...
            val optimizerHints =
              OptimizerHints.empty.withInline(shouldMarkInline)
    ...
    private def moveAllStatementsAfterSuperConstructorCall(
      ...
      assert(
          ...
          "Trying to move a local VarDef after the super constructor call " + "of a Scala.js-defined JS class at ${body.pos}")
    ...
    def genStatOrExpr(tree: Tree, isStat: Boolean): js.Tree = {
      ...
      tree match {
        ...
        case app: Apply => genApply(app, isStat)

        case app: ApplyDynamic => genApplyDynamic(app)
        ...
        case This(qual) =>
          if (tree.symbol == currentClassSym.get) {
            ...
            assert(
                ...
                "Trying to access the this of another class: " + "tree.symbol = " + tree.symbol + ", class symbol = " + currentClassSym.get + " compilation unit:" + currentUnit)
        ...
        case Literal(value) =>
          value.tag match {
            case UnitTag => js.Skip()
            case BooleanTag => js.BooleanLiteral(value.booleanValue)
            case ByteTag | ShortTag | CharTag | IntTag =>
              js.IntLiteral(value.intValue)
            case LongTag => js.LongLiteral(value.longValue)
            case FloatTag => js.FloatLiteral(value.floatValue)
            case DoubleTag => js.DoubleLiteral(value.doubleValue)
            case StringTag => js.StringLiteral(value.stringValue)
            case NullTag => js.Null()
            case ClazzTag => genClassConstant(value.typeValue)
            case EnumTag => genStaticMember(value.symbolValue)
          }

        case tree: Block => genBlock(tree, isStat)

        case Typed(Super(_, _), _) => genThis()
        ...
        case mtch: Match => genMatch(mtch, isStat)

        /** Anonymous function (only with -Ydelambdafy:method) */
        case fun: Function => genAnonFunction(fun)

        case EmptyTree => js.Skip()

        case _ =>
          abort(
              "Unexpected tree in genExpr: " + tree + "/" + tree.getClass + " at: " + tree.pos)
      }
    ...
    def genLabelDef(tree: LabelDef): js.Tree = {
      ...
      tree match {
        ...
        // while (false) { body }
        case LabelDef(lname, Nil, Literal(Constant(()))) => js.Skip()
        ...
        if (catches.isEmpty) {
          ...
          val mightCatchJavaScriptException = catches.exists { caseDef =>
            caseDef.pat match {
              ...
              case pat@Bind(_, _) => isMaybeJavaScriptException(pat.symbol.tpe)
            }
          }
          ...
          val handler0 = catches.foldRight(elseHandler) { (caseDef, elsep) =>
            ...
            val (tpe, boundVar) = (pat match {
              case Typed(Ident(nme.WILDCARD), tpt) => (tpt.tpe, None)
              case Ident(nme.WILDCARD) => (ThrowableClass.tpe, None)
              case Bind(_, _) =>
            ...
            val bodyWithBoundVar = (boundVar match {
              case None => genStatOrExpr(body, isStat)
              case Some(bv) =>
    ...
    def genApply(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      fun match {
        case TypeApply(_, _) => genApplyTypeApply(tree)

        case _ if isRawJSDefaultParam =>
          js.UndefinedParam()(toIRType(sym.tpe.resultType))

        case Select(Super(_, _), _) => genSuperCall(tree, isStat)

        case Select(New(_), nme.CONSTRUCTOR) => genApplyNew(tree)
    ...
    private def genApplyTypeApply(tree: Apply): js.Tree = {
      ...
      val cast = sym match {
        ...
        case _ =>
          abort(
              "Unexpected type application " + fun + "[sym: " + sym.fullName + "]" + " in: " + tree)
      }
      ...
      if (l.isValueType && r.isValueType) {
        ...
        else js.BooleanLiteral(l == r)
      } else if (l.isValueType) {
        val result =
          if (cast) {
            val ctor = ClassCastExceptionClass.info.member(nme.CONSTRUCTOR)
        ...
        else genIsInstanceOf(source, to)
      }
    }
    ...
    private def genApplyNew(tree: Apply): js.Tree = {
      ...
      if (isStringType(tpe)) {
        ...
        toTypeKind(tpe) match {
          case arr@ARRAY(elem) => genNewArray(arr.toIRType, args map genExpr)
    ...
    def genAsInstanceOf(value: js.Tree, to: Type)(
      ...
      def default: js.Tree = js.AsInstanceOf(value, toReferenceType(to))
    ...
    def genNewArray(arrayType: jstpe.ArrayType, arguments: List[js.Tree])(
      ...
      assert(
          ...
          "too many arguments for array constructor: found " + arguments.length + " but array has only " + arrayType.dimensions + " dimension(s)")
      ...
      for (caze@CaseDef(pat, guard, body) <- cases) {
        ...
        def genBody(body: Tree): js.Tree =
           ...
           */
            ...
            case _ => genStatOrExpr(body, isStat)
          }
        ...
        pat match {
          ...
          case Ident(nme.WILDCARD) => elseClause = genDefaultBody
          case Alternative(alts) =>
            val genAlts = {
              alts map {
                ...
                case _ =>
                  abort(
                      "Invalid case in alternative in switch-like pattern match: " + tree + " at: " + tree.pos)
              }
            }
            clauses = (genAlts, genBody(body)) :: clauses
          case _ =>
            abort(
                "Invalid case statement in switch-like pattern match: " + tree + " at: " +
            (tree.pos))
        }
      }
    ...
    private def genBlock(tree: Block, isStat: Boolean): js.Tree = {
      ...
      expr match {
        case expr: LabelDef if isCaseLabelDef(expr) => translateMatch(expr)
        ...
        case _ =>
          assert(
              ...
              "Found stats with case label " + s"def in non-match block at ${tree.pos}: $tree")
      ...
      val translatedCases = for {
        ...
        def genCaseBody(tree: Tree): js.Tree = {
          ...
          tree match {
            ...
            case Apply(_, Nil) if tree.symbol == nextCaseSym => js.Skip()
    ...
    def genOptimizedLabeled(label: js.Ident,
      ...
      def default = js.Labeled(label, tpe, js.Block(translatedCases))

      @tailrec
      def createRevAlts(
          xs: List[js.Tree], acc: List[(js.Tree, js.Tree)]): List[(js.Tree,
          js.Tree)] =
        xs match {
    ...
    private def genPrimitiveOp(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      if (isArithmeticOp(code) || isLogicalOp(code) || isComparisonOp(code))
        ...
        abort(
            "Unknown primitive operation: " + sym.fullName + "(" + fun.symbol.simpleName + ") " + " at: " +
        (tree.pos))
    }
      ...
      sources match {
        ...
        case List(source) =>
          (code match {
            ...
            case ZNOT => js.UnaryOp(js.UnaryOp.Boolean_!, source)
          ...
          def genEquality(eqeq: Boolean, not: Boolean) = {
            opType match {
              case jstpe.IntType | jstpe.DoubleType | jstpe.FloatType =>
                js.BinaryOp(if (not) js.BinaryOp.Num_!=
                            else js.BinaryOp.Num_==,
                            lsrc,
                            rsrc)
              ...
              case _ =>
                if (eqeq &&
                  ...
                  if (not) js.UnaryOp(js.UnaryOp.Boolean_!, body)
                  else body
                } else {
                  js.BinaryOp(if (not) js.BinaryOp.!==
                              else js.BinaryOp.===,
                              lsrc,
                              rsrc)
                }
    ...
    private def genCoercion(tree: Apply, receiver: Tree, code: Int): js.Tree = {
      ...
      (code: @switch) match {
        ...
        case L2I | F2I | D2I => source2int

        // Any int to Long
        case C2L | B2L | S2L | I2L => js.UnaryOp(js.UnaryOp.IntToLong, source)

        // Any double to Long
        case F2L | D2L => js.UnaryOp(js.UnaryOp.DoubleToLong, source)

        // Long to Double
        case L2D => js.UnaryOp(js.UnaryOp.LongToDouble, source)
        ...
        case C2C | B2B | S2S | I2I | L2L | F2F | D2D | C2I | C2D | B2S | B2I | B2D | S2I | S2D | I2D | F2D =>
          source
      }
    }
    ...
    private def genApplyDynamic(tree: ApplyDynamic): js.Tree = {
      ...
      if (isEqOrNeq) {
        ...
        val op =
          if (sym.name == nme.eq) js.BinaryOp.===
          else js.BinaryOp.!==
    ...
    def ensureBoxed(expr: js.Tree, tpeEnteringPosterasure: Type)(
        implicit pos: Position): js.Tree = {
      tpeEnteringPosterasure match {
        case tpe if isPrimitiveValueType(tpe) => makePrimitiveBox(expr, tpe)
    ...
    def fromAny(expr: js.Tree, tpeEnteringPosterasure: Type)(
        implicit pos: Position): js.Tree = {
      tpeEnteringPosterasure match {
        case tpe if isPrimitiveValueType(tpe) => makePrimitiveUnbox(expr, tpe)
        ...
        case tpe => genAsInstanceOf(expr, tpe)
      }
    }
    ...
    private def genJSPrimitive(
      ...
      if (code == DYNNEW) {
        ...
        def warnIfDuplicatedKey(pairs: List[(js.StringLiteral,
            js.Tree)]): Unit = {
        ...
        def fail() = {
          reporter.error(
              pos,
              "runtime.constructorOf() must be called with a constant " + "classOf[T] representing a class extending js.Any " + "(not a trait nor an object)")
          js.Undefined()
        }
        args match {
          ...
          case List(arg) =>
            ...
            code match {
              ...
              case F2JS =>
                 ...
                 */
                  case JSFunctionToScala(fun, arity) => fun
                  case _ => genFunctionToJSFunction(isThisFunction = false)
                }

              /** Convert a scala.FunctionN f to a js.ThisFunction{N-1}. */
              case F2JSTHIS => genFunctionToJSFunction(isThisFunction = true)
    ...
    private def genJSSuperCall(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      if (sym.owner == ObjectClass) {
        ...
        assert(
            ...
            "Trying to call the super constructor of Object in a " + s"Scala.js-defined JS class at $pos")
    ...
    private def genJSCallGeneric(sym: Symbol,
      ...
      val boxedResult = sym.name match {
        ...
        case _ =>
          ...
          if (jsInterop.isJSGetter(sym)) {
            ...
            args match {
              case List(keyArg) => genSelectGet(keyArg)
              case List(keyArg, valueArg) => genSelectSet(keyArg, valueArg)
            }
      ...
      boxedResult match {
        case js.Assign(_, _) => boxedResult
        case _ if isStat => boxedResult
        case _ =>
          fromAny(
              boxedResult,
    ...
    private object JSBinaryOpMethodName {
      ...
      def unapply(name: TermName): Option[js.JSBinaryOp.Code] = map.get(name)
    }
     ...
     */
    private def extractFirstArg(args: List[js.Tree]): (js.Tree,
        List[js.Tree]) = {
      ...
      assert(
          ...
          "Trying to extract the first argument of an argument list starting " + "with a Spread argument: " + firstArg)
      (firstArg, args.tail)
    }
    ...
    private def genActualArgs(sym: Symbol, args: List[Tree])(
      ...
      if (wereRepeated.size > args.size) {
         ...
         */
        for ((arg, wasRepeated) <- args
          .zipAll(wereRepeated, EmptyTree, false)) yield {
          if (wasRepeated) {
    ...
    private def genPrimitiveJSArgs(sym: Symbol, args: List[Tree])(
      ...
      val paramTpes = enteringPhase(currentRun.posterasurePhase) {
        for (param <- sym.tpe.params) yield param.name -> param.tpe
      }.toMap
    ...
    private def tryGenRepeatedParamAsJSArray(
      ...
      arg match {
        ...
        case MaybeAsInstanceOf(WrapArray(MaybeAsInstanceOf(ArrayValue(
            tpt, elems)))) =>
        ...
        // foo()
        case Select(_, _) if handleNil && arg.symbol == NilModule => Some(Nil)
    ...
    private def tryGenAndRecordAnonFunctionClassGeneric(cd: ClassDef)(
      ...
      withNewLocalNameScope {
        ...
        if (paramAccessors.size != ctorParams.size &&
          ...
          fail(
              s"Have param accessors $paramAccessors but " + s"ctor params $ctorParams in anon function $cd")
        }
        ...
        val applyMethod = withScopedVars(
          ...
          try {
            genMethodWithCurrentLocalNameScope(applyDef).getOrElse(abort(
                s"Oops, $applyDef did not produce a method"))
          } catch {
            case e: CancelGenMethodAsJSFunction => fail(e.getMessage)
          }
        }
    ...
    private def genAnonFunction(originalFunction: Function): js.Tree = {
      ...
      val Function(paramTrees, Apply(targetTree@Select(receiver, _), allArgs0)) =
        originalFunction
    ...
    private def patchFunBodyWithBoxes(
      ...
      val paramTpes = enteringPhase(currentRun.posterasurePhase) {
        for (param <- methodType.params) yield param.name -> param.tpe
      }.toMap
    ...
    def genLoadModule(sym0: Symbol)(implicit pos: Position): js.Tree = {
      ...
      val sym1 =
        if (sym0.isModule) sym0.moduleClass
        else sym0
  ...
  private def isStringType(tpe: Type): Boolean = tpe.typeSymbol == StringClass

  private def isLongType(tpe: Type): Boolean = tpe.typeSymbol == LongClass
