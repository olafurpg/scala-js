...
abstract class PrepJSInterop
  ...
  class JSInteropTransformer(unit: CompilationUnit)
      extends Transformer {
      ...
      postTransform {
        tree match {
          ...
          case idef: ImplDef if enclosingOwner is OwnerKind.JSNativeClass =>
            reporter.error(
                idef.pos,
                "Native JS traits and classes " + "may not have inner traits, classes or objects")
       ...
       */
          ...
          case cldef: ClassDef =>
            ...
            if (sym.hasAnnotation(JSNativeAnnotation)) {
              reporter.error(
                  cldef.pos,
                  "Traits and classes not extending js.Any " + "may not have a @js.native annotation")
            }
          ...
          case modDef: ModuleDef =>
            ...
            if (sym.hasAnnotation(JSNativeAnnotation)) {
              reporter.error(
                  modDef.pos,
                  "Objects not extending js.Any may not " + "have a @js.native annotation")
            }
          ...
          case TypeApply(classOfTree@Select(predef, nme.classOf), List(tpeArg))
            ...
            if (scalaJSOpts.fixClassOf) {
              ...
              if (typer.checkClassType(tpeArg)) {
                ...
                reporter.error(
                    tpeArg.pos, s"Type ${tpeArg} is not a class type")
                EmptyTree
              }
            } else {
              reporter.error(
            ...
            for {
              ...
              val msg = {
                ...
                if (sym.owner.isPrimaryConstructor)
                  base + ". To export a (case) class field, use the " + "meta-annotation scala.annotation.meta.field like this: " + "@(JSExport @field)."
                else base
              }
    ...
    private def transformJSAny(implDef: ImplDef) = {
      ...
      if (sym.isPackageObjectClass) {
        reporter.warning(
            implDef.pos,
            "Package objects inheriting from js.Any are deprecated. " + "Use a normal object instead.")
      } else if (isJSNative && !isJSAnonFun &&
                 !sym.hasAnnotation(JSNativeAnnotation)) {
        reporter.warning(
            implDef.pos,
            "Classes, traits and objects inheriting from js.Any should be " + "annotated with @js.native, unless they have @ScalaJSDefined. " + "The default will switch to Scala.js-defined in the next major " + "version of Scala.js.")
      ...
      if (!isJSAnonFun && badParent.isDefined) {
        ...
        reporter.error(
            implDef.pos,
            s"${sym.nameString} extends ${badName} " + "which does not extend js.Any.")
      }
      ...
      if (!isJSNative) {
        ...
        if (!sym.isTrait && (enclosingOwner is OwnerKind.JSNativeMod)) {
          reporter.error(
              implDef.pos,
              "Native JS objects cannot contain inner Scala.js-defined JS " + "classes or objects")
        }
        ...
        if (!sym.isTrait && sym.info.parents.exists(_ =:= AnyRefClass.tpe)) {
          reporter.error(
              implDef.pos,
              s"A Scala.js-defined JS $strKind cannot directly extend AnyRef. " + "It must extend a JS class (native or not).")
        }
        ...
        if (sym.info.parents.exists(isNativeJSTraitType)) {
          reporter.error(
              implDef.pos,
              s"A Scala.js-defined JS $strKind cannot directly extend a " + "native JS trait.")
        }
      }
      ...
      if (isJSNative && !isJSAnonFun) {
        ...
        if (sym.isLocalToBlock) {
          ...
          reporter.error(
              implDef.pos,
              "Traits and classes " + "may not have inner native JS traits, classes or objects")
        } else if (enclosingOwner is OwnerKind.JSMod) {
          reporter.error(
              implDef.pos,
              "Scala.js-defined JS objects " + "may not have inner native JS classes or objects")
          ...
          if (enclosingOwner is OwnerKind.ScalaMod) {
            if (sym.isModuleClass) {
              reporter.error(
                  implDef.pos,
                  "Native JS objects inside " + "non-native objects must have an @JSName annotation")
            } else {
              // This should be an error, but we erroneously allowed that before
              reporter.warning(
                  implDef.pos,
                  "Native JS classes inside " + "non-native objects should have an @JSName annotation. " + "This will be enforced in 1.0.")
            }
          }
        }
      }
        ...
        if (jsInterop.jsNameOf(low) != jsInterop.jsNameOf(high)) {
          ...
          val msg = {
            ...
            def memberDefString(membSym: Symbol) = {
              membSym
                .defStringSeenAs(sym.thisType.memberType(membSym)) + membSym.locationString + " with JSName '" + jsInterop
                .jsNameOf(membSym) + '\''
            }
    ...
    private def transformValOrDefDefInRawJSType(tree: ValOrDefDef) = {
      ...
      if (shouldPrepareExports) {
        ...
        lazy val memType =
          if (sym.isConstructor) "constructor"
          else "method"
        for {
          exp <- exportsOf(sym) if !exp.ignoreInvalid
        } {
      ...
      if (sym.name == nme.apply && !sym.hasAnnotation(JSNameAnnotation)) {
        if (jsInterop.isJSGetter(sym)) {
          reporter.error(
              sym.pos,
              s"A member named apply represents function " + "application in JavaScript. A parameterless member should be " + "exported as a property. You must add @JSName(\"apply\")")
        } else if (enclosingOwner is OwnerKind.JSNonNative) {
          reporter.error(
              sym.pos,
              "A Scala.js-defined JavaScript class cannot declare a method " + "named `apply` without `@JSName`")
        }
      }
      ...
      if (enclosingOwner is OwnerKind.JSNonNative) {
        ...
        if (sym.isMethod && isPrivateMaybeWithin(sym)) {
          ...
          if (alts.isOverloaded) {
            reporter.error(
                tree.pos,
                "Private methods in Scala.js-defined JS classes cannot be " + "overloaded. Use different names instead.")
          }
        }
        ...
        if (sym.isMethod && (sym.hasAccessBoundary && !sym.isProtected) &&
          ...
          reporter.error(
              tree.pos,
              "Qualified private members in Scala.js-defined JS classes " + "must be final")
        }
      ...
      if (sym.isPrimaryConstructor || sym.isValueParameter ||
        ...
        tree.rhs match {
          ...
          case _ =>
            reporter.error(
                tree.pos,
                "A secondary constructor of a class " + "extending js.Any may only call the primary constructor")
        }
      } else {
        ...
        tree.rhs match {
          ...
          case _ =>
            val pos =
              if (tree.rhs != EmptyTree) tree.rhs.pos
              else tree.pos
            reporter.warning(
                pos,
        ...
        if (sym.tpe.resultType.typeSymbol == NothingClass &&
          ...
          reporter.warning(
              tree.pos,
              s"The type of $name got inferred " + "as Nothing. To suppress this warning, explicitly ascribe " + "the type.")
        }
      }
  ...
  private def primCtorNoArg(cldef: ClassDef) =
    getPrimCtor(cldef.symbol.tpe).map(_.paramss == List(List()))
      .getOrElse(true)
  ...
  private def fixPublicBeforeTyper(ddef: DefDef): DefDef = {
    ...
    val sym = ddef.symbol
    val needsFix = {
      sym.isPrivate && (wasPublicBeforeTyper(sym) || (sym.isAccessor &&
              wasPublicBeforeTyper(sym.accessed)))
    }
    if (needsFix) {
