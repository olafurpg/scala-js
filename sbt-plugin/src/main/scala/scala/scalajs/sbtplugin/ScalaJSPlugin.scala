...
object ScalaJSPlugin extends AutoPlugin {
  ...
  object AutoImport extends impl.DependencyBuilders
    ...
    val packageMinifiedJSDependencies = TaskKey[File](
        ...
        "Packages minified version (if available) of dependencies of the preLink " + "classpath in a single file.",
        AMinusTask)
    ...
    val scalaJSLinkedFile = TaskKey[VirtualJSFile](
        "scalaJSLinkedFile",
        "Linked Scala.js file. This is the result of fastOptJS or fullOptJS, " + "depending on the stage.",
        DTask)
    ...
    val resolvedJSEnv = TaskKey[JSEnv](
        "resolvedJSEnv",
        "The JSEnv used for execution. This equals the setting of jsEnv or a " + "reasonable default value if jsEnv is not set.",
        DTask)
    ...
    val checkScalaJSSemantics = SettingKey[Boolean](
        ...
        "Whether to check that the current semantics meet compliance " + "requirements of dependencies.",
        CSetting)

    val persistLauncher = SettingKey[Boolean](
        "persistLauncher",
        "Tell optimize/package tasks to write the laucher file to disk. " + "If this is set, your project may only have a single mainClass or you must explicitly set it",
        AMinusSetting)
    ...
    val scalaJSPhantomJSClassLoader = TaskKey[ClassLoader](
        ...
        "Private class loader to load jetty8 without polluting classpath. Only use this " + "as the `jettyClassLoader` argument of the PhantomJSEnv",
        KeyRanks.Invisible)
