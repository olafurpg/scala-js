...
class Reversi(jQuery: JQueryStatic, playground: JQuery) {
  ...
  def buildUI() {
        ...
        def drawSquare(square: Square) {
              ...
              if (square.owner != NoPlayer) {
                context.fillStyle = if (square.owner == White) "white"
                else "black"
                context.beginPath()
  ...
  def startTurn() {
        ...
        status.text(
            currentPlayer + "'s turn -- White: " + scoreWhite + " -- Black: " + scoreBlack)
        ...
        if (!existsValidMove()) {
          ...
          if (opponentCanDoSomething) {
            ...
            status.text(
                "Game finished -- White: " + scoreWhite + " -- Black: " + scoreBlack + " -- " + winnerText)
          }
        }
      }
