...
object TestUtils {
  ...
  def newInstance (name: String,
      loader: ClassLoader)(args: Seq[AnyRef]): AnyRef = macro newInstance_impl

  def newInstance_impl(c: Context)(
      name: c.Expr[String], loader: c.Expr[ClassLoader])(
  ...
  def loadModule (name: String,
      loader: ClassLoader): AnyRef = macro loadModule_impl

  def loadModule_impl(c: Context)(
      name: c.Expr[String], loader: c.Expr[ClassLoader]): c.Expr[AnyRef] =
    c.universe.reify {
      val clazz = loader.splice.loadClass(name.splice + "$")
