...
final class BaseLinker(
  ...
  @deprecated(
      "Bypassing linking errors will not be possible in the next major version. " + "Use the overload without the bypassLinkingError parameter instead.",
      "0.6.6")
  def link(irInput: Seq[VirtualScalaJSIRFile],
           logger: Logger,
           symbolRequirements: SymbolRequirement,
  ...
  private def linkInternal(infoInput: List[Infos.ClassInfo],
    ...
    if (analysis.errors.nonEmpty) {
      ...
      val fatal =
        !bypassLinkingErrors || analysis.errors.exists {
  ...
  private def linkedClassDef(info: Infos.ClassInfo,
    ...
    // Synthetic members
    for {
      m <- analyzerInfo.methodInfos.valuesIterator if m.isReachable
    } {
      m.syntheticKind match {
        case MethodSyntheticKind.None => // nothing to do
