  ...
  private def makeContent(
    ...
    val content1 = outputMode match {
      ...
      case OutputMode.ECMAScript51Isolated | OutputMode.ECMAScript6 =>
        content.replaceAll("ScalaJS\\.d\\.", "\\$d_").replaceAll(
            "ScalaJS\\.c\\.",
            "\\$c_").replaceAll("ScalaJS\\.h\\.", "\\$h_").replaceAll(
            "ScalaJS\\.s\\.",
            "\\$s_").replaceAll("ScalaJS\\.n\\.", "\\$n_").replaceAll(
            "ScalaJS\\.m\\.", "\\$m_").replaceAll("ScalaJS\\.is\\.", "\\$is_")
