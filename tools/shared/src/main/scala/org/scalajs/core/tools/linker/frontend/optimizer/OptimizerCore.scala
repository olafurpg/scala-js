...
import scala.util.control.TailCalls.{done => _,
_} // done is a too generic term
...
private [optimizer] abstract class OptimizerCore(
  ...
  private def transform(tree: Tree, isStat: Boolean)(
    ...
    val result = tree match {
          ...
          pretransformExpr(expr) { texpr =>
            val result = {
              ...
              if (isSubtype(texpr.tpe.base, tpe.asInstanceOf[Type])) {
                ...
                else Block(finishTransformStat(texpr), BooleanLiteral(true))
              } else {
                if (texpr.tpe.isExact)
                  Block(finishTransformStat(texpr), BooleanLiteral(false))
                else IsInstanceOf(finishTransformExpr(texpr), tpe)
              }
            }
          ...
          pretransformExpr(expr) { texpr =>
            texpr.tpe match {
              case RefinedType(base: ReferenceType, true, false) =>
                TailCalls.done(Block(finishTransformStat(texpr),
                                     ClassOf(base)))
      ...
      case _: Skip | _: Debugger | _: LoadModule | _: LoadJSConstructor |
          _: LoadJSModule | _: JSEnvInfo | _: JSLinkingInfo |
          _: Literal | EmptyTree =>
        tree
  ...
  private def pretransformExpr(tree: Tree)(cont: PreTransCont)(
    ...
    tailcall {
      ...
      tree match {
        ...
        case tree: ApplyStatically =>
          pretransformStaticApply(
              tree, isStat = false, usePreTransform = true)(cont)

        case tree: ApplyStatic =>
          pretransformApplyStatic(
              tree, isStat = false, usePreTransform = true)(cont)
            ...
            tpe match {
              ...
              case _ =>
                ...
                if (isSubtype(texpr.tpe.base, tpe.asInstanceOf[Type])) {
                  ...
                  cont(PreTransTree(AsInstanceOf(finishTransformExpr(texpr),
                                                 tpe)))
                }
            }
          }
  ...
  private def pretransformSelectCommon(expectedType: Type,
    ...
    preTransQual match {
      case PreTransLocalDef(LocalDef(_,
                                     _,
  ...
  private def pretransformApply(
    ...
    def treeNotInlined =
      cont(PreTransTree(Apply(finishTransformExpr(treceiver),
                              methodIdent,
            ...
            if (impls.forall(_.isForwarder)) {
              ...
              val areAllTheSame = getMethodBody(reference).body match {
                ...
                case MaybeBox(
                  ...
                  impls.tail.forall(getMethodBody(_).body match {
                    case MaybeBox(Apply(
                                  This(), Ident(`methodName`, _), implArgs),
                                  `boxID`) =>
  ...
  private def pretransformStaticApply(
    ...
    def treeNotInlined0(
      ...
      cont(PreTransTree(ApplyStatically(transformedReceiver,
                                        clsType,
                                        methodIdent,
  ...
  private def pretransformApplyStatic(
      ...
      cont: PreTransCont)(implicit scope: Scope): TailRec[Tree] = {
    val ApplyStatic(
    classType@ClassType(cls), methodIdent@Ident(methodName, _), args) = tree
    implicit val pos = tree.pos
    ...
    if (optTarget.isEmpty) {
      ...
      pretransformExprs(args) { targs =>
        ...
        if (intrinsicCode >= 0) {
          ...
          val shouldInline =
            target.inlineable && (target.shouldInline ||
                shouldInlineBecauseOfArgs(target, targs))
  ...
  private def pretransformJSFunctionApply(
    ...
    if (args.exists(_.isInstanceOf[JSSpread])) {
      cont(PreTransTree(JSFunctionApply(transformExpr(fun),
                                        args.map(transformExprOrSpread))))
    } else {
      ...
      case Assign(lhs@Select(This(), field), VarRef(Ident(rhsName, _)))
        ...
        pretransformSelectCommon(
          ...
          preTransLhs =>
            ...
            cont(PreTransTree(Assign(finishTransformExpr(preTransLhs),
      ...
      case ClassGetComponentType =>
        newReceiver match {
          case ClassOf(ArrayType(base, depth)) =>
            contTree(ClassOf(if (depth == 1) ClassType(base)
        ...
        (initialFieldLocalDefList, cont1) =>
          ...
          inlineClassConstructorBody(allocationSite,
            ...
            (finalFieldLocalDefs, cont2) =>
              cont2(PreTransLocalDef(
                  LocalDef(RefinedType(cls,
                                       isExact = true,
                                       isNullable = false,
                                       allocationSite = Some(allocationSite)),
                           mutable = false,
                           InlineClassInstanceReplacement(recordType,
                                                          finalFieldLocalDefs,
                                                          cancelFun))))
          }(cont1)
      }(cont)
  ...
  private def finishTransformOptLongExpr(targ: PreTransform): Tree =
    targ match {
      case PreTransLocalDef(LocalDef(
          tpe, false, ReplaceWithVarRef(_, _, _, Some(argValue)))) =>
        argValue()
            ...
            staticCall(LongImpl.RuntimeLongClass, LongImpl.hi).fold[Tree] {
              ...
              trampoline {
                ...
                inline(Nil,
                       ...
                       target,
                       isStat = false,
                       usePreTransform = false)(finishTransform(
                    isStat = false))
              }
            }
    ...
    (op: @switch) match {
      ...
      case Long_/ =>
        (lhs, rhs) match {
          ...
          case (LongFromInt(x), LongFromInt(y: IntLiteral))
              if y.value != - 1 =>
        ...
        (lhs, rhs) match {
          ...
          case (_, LongLiteral(Long.MinValue)) =>
            if (op == Long_< || op == Long_>=)
              ...
              foldBinaryOp(if (op == Long_<=) Long_==
                           else Long_!=,
                           lhs,
                           rhs)
          ...
          case (BinaryOp(Long_+, LongLiteral(x), y@LongFromInt(_)),
                LongLiteral(z)) if canAddLongs(x, Int.MinValue) &&
           ...
           */
          case (BinaryOp(Long_-, LongLiteral(x), y@LongFromInt(_)),
                LongLiteral(z)) if canSubtractLongs(x, Int.MinValue) &&
            ...
            trampoline {
              withNewLocalDefs(
                ...
                (tempsLocalDefs, cont) =>
                  ...
                  cont(PreTransTree(
                      AndThen(AndThen(BinaryOp(Num_>, tempX, IntLiteral(0)),
  ...
  private def foldUnbox(arg: PreTransform, charCode: Char)(
    ...
    (charCode: @switch) match {
      ...
      case 'D' if arg.tpe.base == DoubleType || arg.tpe.base == IntType ||
  ...
  private def foldReferenceEquality(
    ...
    (tlhs, trhs) match {
      ...
      case _ =>
        foldBinaryOp(if (positive) BinaryOp.===
                     else BinaryOp.!==,
                     finishTransformExpr(tlhs),
                     finishTransformExpr(trhs))
    }
  ...
  private def returnable(oldLabelName: String,
      ...
      implicit scope: Scope, pos: Position): TailRec[Tree] =
    tailcall {
      val newLabel = freshLabelName(if (oldLabelName.isEmpty) "inlinereturn"
      else oldLabelName)
      ...
      def doMakeTree(newBody: Tree, returnedTypes: List[Type]): Tree = {
        ...
        tryOptimizePatternMatch(
            oldLabelName, refinedType, returnCount, newBody) getOrElse {
      ...
      val info = new LabelInfo(newLabel,
                               ...
                               returnedTypes = newSimpleState(Nil))
      val bodyScope =
        scope.withEnv(scope.env.withLabelInfo(oldLabelName, info))
          ...
          pretransformExpr(body) { tbody0 =>
            ...
            if (returnedTypes0.isEmpty) {
              ...
              actualTypes.collectFirst {
                ...
                cont(PreTransRecordTree(resultTree,
                                        refinedOrigType,
                                        cancelFun))
              }
            }
...
private [optimizer] object OptimizerCore {
  ...
  private final class LabelInfo(val newName: String,
                                val acceptRecords: Boolean,
                                /** (actualType, originalType), actualType can be a RecordType. */
                                val returnedTypes: SimpleState[List[(Type,
                                RefinedType)]])
  ...
  private class Scope(val env: OptEnv,
                      val implsBeingInlined: Set[(List[Option[AllocationSite]],
                      AbstractMethodID)]) {
  ...
  private object PreTransBlock {
    ...
    def unapply(preTrans: PreTransBlock): Some[(List[Tree],
        PreTransLocalDef)] = Some(preTrans.stats, preTrans.result)
  }
  ...
  private def exceptionMsg(myself: AbstractMethodID,
    ...
    buf.append(
        "The Scala.js optimizer crashed while optimizing " + myself + ": " + cause.toString)
