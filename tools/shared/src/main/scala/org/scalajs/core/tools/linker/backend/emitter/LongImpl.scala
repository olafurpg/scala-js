...
private [linker] object LongImpl {
  ...
  final val divideUnsigned =
    "divideUnsigned__sjsr_RuntimeLong__sjsr_RuntimeLong"
  final val remainderUnsigned =
    "remainderUnsigned__sjsr_RuntimeLong__sjsr_RuntimeLong"

  val AllIntrinsicMethods = Set.empty[String]

  // TODO Move these to AllIntrinsicMethods when we can break binary compatibility
  val OptionalIntrinsicMethods = Set(divideUnsigned, remainderUnsigned)
   ...
   */
  val AllConstructors = Set(initFromPartsOld, initFromInt)
  ...
  val AllModuleMethods = Set(fromDouble, Zero)
