...
object DependencyResolver {
  type DependencyFilter = Traversable[FlatJSDependency] => Traversable[FlatJSDependency]
  ...
  def resolveDependencies(
    ...
    val resolvedJSLibs = resolveAllResourceNames(
        resourceNames, availableLibs.keys)
  ...
  private def collectAllResourceNames(
      manifests: Traversable[JSDependencyManifest]): Map[String,
      List[Origin]] = {
  ...
  private def resolveAllResourceNames(
    ...
    for ((resourceName, origins) <- allResourceNames) {
      resolveResourceName(resourceName, origins, relPaths).fold[Unit](
          problems += _, resolvedLibs += resourceName -> _)
    }
  ...
  private def checkMinifiedJSConflicts(byRelPath: Map[String,
      Traversable[FlatJSDependency]]) = {
    @inline
    def hasConflict(x: FlatJSDependency, y: FlatJSDependency) =
