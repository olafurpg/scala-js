...
private [scalajs] final class ScalaJSClassEmitter(
    private [emitter] val outputMode: OutputMode, linkingUnit: LinkingUnit) {
  ...
  def genClass(tree: LinkedClass): js.Tree = {
    ...
    val memberDefs = tree.memberMethods.map(m => genMethod(className, m.tree))
  ...
  def genES6Class(tree: LinkedClass, members: List[js.Tree]): js.Tree = {
    ...
    val className = tree.name.name
    val classIdent =
      encodeClassVar(className)(outputMode, tree.name.pos).asInstanceOf[
          js.VarRef].ident
  ...
  def genMethod(className: String, method: MethodDef): js.Tree = {
    ...
    outputMode match {
      case OutputMode.ECMAScript6StrongMode =>
        js.MethodDef(static = method.static,
  ...
  private def genPropertyES5(
    ...
    val defProp = js.BracketSelect(
        js.VarRef(js.Ident("Object")), js.StringLiteral("defineProperty"))
      ...
      envFieldDef(
          ...
          js.Function(List(objParam, depthParam), className match {
            case Definitions.ObjectClass =>
              ...
              js.Block(if (isStrongMode) js.Skip()
                       else dataVarDef,
                       js.If(!(if (isStrongMode) genIsScalaJSObject(obj)
                               else data), {
  ...
  private def genIsClassNameInAncestors(className: String, ancestors: js.Tree)(
    ...
    else js.BinaryOp(JSBinaryOp.in, js.StringLiteral(className), ancestors)
  }
  ...
  def genTypeData(tree: LinkedClass): js.Tree = {
    ...
    val isObjectClass = className == ObjectClass
    ...
    val isRawJSType = kind == ClassKind.RawJSType || kind.isJSClass
    ...
    val prunedParams = outputMode match {
      case OutputMode.ECMAScript6StrongMode => allParams
      case _ =>
    ...
    outputMode match {
      ...
      case _ => envFieldDef("d", className, typeData)
    }
  }
  ...
  def genSetTypeData(tree: LinkedClass): js.Tree = {
    ...
    encodeClassVar(tree.name.name).prototype DOT "$classData" := envField(
        "d", tree.name.name)
  }
  ...
  def genModuleAccessor(tree: LinkedClass): js.Tree = {
    ...
    val createModuleInstanceField = outputMode match {
      case OutputMode.ECMAScript6StrongMode => js.Skip()
      case _ => envFieldDef("n", className, js.Undefined(), mutable = true)
    }
    ...
    val createAccessor = {
      ...
      val initBlock = semantics.moduleInit match {
        ...
        case CheckedBehavior.Fatal =>
          js.If(moduleInstanceVar === js.Undefined(), {
            ...
            val msg =
              s"Initializer of $decodedName called before completion " + "of its super constructor"
            val obj =
      ...
      outputMode match {
        ...
        case _ => envFieldDef("m", className, js.Function(Nil, body))
      }
    }
  ...
  def genConstructorExportDef(
    ...
    outputMode match {
      case OutputMode.ECMAScript6StrongMode =>
        ...
        js.Apply(js.VarRef(js.Ident("$exportCtor")),
                 List(nsParts, name, exportedCtor, baseCtor DOT "prototype"))

      case _ =>
        val (createNamespace, expCtorVar) = genCreateNamespaceInExports(
            fullName)
        js.Block(
            createNamespace,
            js.DocComment("@constructor"),
  ...
  def genModuleExportDef(cd: LinkedClass, tree: ModuleExportDef): js.Tree = {
    ...
    val baseAccessor = outputMode match {
      ...
      case _ => envField("m", cd.name.name)
    }
  ...
  private def genClassOrModuleExportDef(
    ...
    outputMode match {
      case OutputMode.ECMAScript6StrongMode =>
        ...
        js.Apply(
            js.VarRef(js.Ident("$export")), List(nsParts, name, exportedValue))
...
private object ScalaJSClassEmitter {
  private val ClassesWhoseDataReferToTheirInstanceTests = {
    Definitions.AncestorsOfHijackedClasses + Definitions.ObjectClass + Definitions.StringClass
  }
}
