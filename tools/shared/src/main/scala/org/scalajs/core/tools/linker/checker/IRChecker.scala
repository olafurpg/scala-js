...
private final class IRChecker(unit: LinkingUnit, logger: Logger) {
    ...
    if (classDef.kind != ClassKind.HijackedClass &&
      ...
      for (member <- classDef.exportedMembers) {
        ...
        member.tree match {
          ...
          case _ =>
            reportError(
                "Illegal exported class member of type " + member.tree.getClass.getName)
        }
      }
  ...
  private def checkMethodDef(
      methodDef: MethodDef, classDef: LinkedClass): Unit = {
    ...
    if (advertizedSig != sigFromName) {
      reportError(
          s"The signature of ${classDef.name.name}.$name, which is " + s"$advertizedSig, does not match its name (should be $sigFromName).")
    }
  ...
  private def checkExportedMethodDef(
      methodDef: MethodDef, classDef: LinkedClass): Unit = {
    ...
    for (ParamDef(name, tpe, _, _) <- params) {
      if (tpe == NoType)
        ...
        reportError(
            s"Parameter $name of exported method def has type $tpe, " + "but must be Any")
    }
    ...
    if (classDef.kind.isJSClass && name == "constructor") {
      ...
      if (resultType != AnyType) {
        reportError(
            s"Result type of exported method def is $resultType, " + "but must be Any")
      }
  ...
  private def checkJSClassConstructor(
      methodDef: MethodDef, classDef: LinkedClass): Unit = {
    ...
    if (resultType != NoType) {
      reportError(
          s"Result type of JS class constructor is $resultType, " + "but must be NoType")
    }
    ...
    val (superCall, restStats) = superCallAndRest match {
      ...
      case _ =>
        reportError(
            "A JS class constructor must contain one super constructor call " + "at the top-level")
  ...
  private def checkExportedPropertyDef(
      propDef: PropertyDef, classDef: LinkedClass): Unit = {
    ...
    if (setterBody != EmptyTree) {
      if (setterArg.ptpe != AnyType)
        reportError(
            "Setter argument of exported property def has type " + s"${setterArg.ptpe}, but must be Any")
      if (setterArg.rest)
  ...
  private def checkConstructorExportDef(
      ctorDef: ConstructorExportDef, classDef: LinkedClass): Unit = {
    ...
    for (ParamDef(name, tpe, _, _) <- params) {
      if (tpe == NoType)
        ...
        reportError(
            s"Parameter $name of exported constructor def has type " + s"$tpe, but must be Any")
    }
  ...
  private def checkJSClassExportDef(
      classExportDef: JSClassExportDef, classDef: LinkedClass): Unit = {
  ...
  private def checkModuleExportDef(
      moduleDef: ModuleExportDef, classDef: LinkedClass): Unit = {
  ...
  private def typecheckStat(tree: Tree, env: Env): Env = {
    ...
    tree match {
      ...
      case Assign(select, rhs) =>
        select match {
          case Select(This(), Ident(_, _)) if env.inConstructor => // ok
  ...
  private def typecheckExpect(tree: Tree, env: Env, expectedType: Type)(
    ...
    if (!isSubtype(tpe, expectedType))
      reportError(
          s"$expectedType expected but $tpe found " + s"for tree of type ${tree.getClass.getName}")
  }
  ...
  private def typecheck(tree: Tree, env: Env): Type = {
    ...
    def checkApplyGeneric(methodName: String,
      ...
      if (args.size != methodParams.size)
        reportError(
            s"Arity mismatch: ${methodParams.size} expected but " + s"${args.size} found")
      for ((actual, formal) <- args zip methodParams) {
        typecheckExpect(actual, env, formal)
      }
      if (tpe != resultType)
        reportError(
            s"Call to $methodFullName of type $resultType " + s"typed as ${tree.tpe}")
    }
    ...
    tree match {
      ...
      case New(cls, ctor, args) =>
        ...
        checkApplyGeneric(
            ctor.name, s"$cls.$ctor", args, NoType, isStatic = false)
              ...
              maybeClass.right foreach {
                _.lookupField(item).fold[Unit] {
                  ...
                  if (fieldDef.tpe != tree.tpe)
                    reportError(
                        s"Select $cls.$item of type " + s"${fieldDef.tpe} typed as ${tree.tpe}")
                }
              }
            }
          case NullType | NothingType => // always ok
          case _ =>
      ...
      case Apply(receiver, Ident(method, _), args) =>
        ...
        checkApplyGeneric(
            method, s"$receiverType.$method", args, tree.tpe, isStatic = false)

      case ApplyStatically(receiver, cls, Ident(method, _), args) =>
        typecheckExpect(receiver, env, cls)
        checkApplyGeneric(
            method, s"$cls.$method", args, tree.tpe, isStatic = false)

      case ApplyStatic(cls, Ident(method, _), args) =>
        val clazz = lookupClass(cls)
        checkApplyGeneric(
            method, s"$cls.$method", args, tree.tpe, isStatic = true)
      ...
      case BinaryOp(op, lhs, rhs) =>
        ...
        (op: @switch) match {
          ...
          case Int_+ | Int_- | Int_* | Int_/ | Int_% | Int_| | Int_& | Int_^ | Int_<< | Int_>>> | Int_>> =>
          ...
          case Float_+ | Float_- | Float_* | Float_/ | Float_% =>
            ...
            typecheckExpect(lhs, env, FloatType)
          case Long_+ | Long_- | Long_* | Long_/ | Long_% | Long_| | Long_& | Long_^ | Long_== | Long_!= | Long_< | Long_<= | Long_> | Long_>= =>
            typecheckExpect(lhs, env, LongType)
            typecheckExpect(rhs, env, LongType)
          case Long_<< | Long_>>> | Long_>> =>
            typecheckExpect(lhs, env, LongType)
            typecheckExpect(rhs, env, IntType)
          case Double_+ | Double_- | Double_* | Double_/ | Double_% | Num_== | Num_!= | Num_< | Num_<= | Num_> | Num_>= =>
            typecheckExpect(lhs, env, DoubleType)
            typecheckExpect(lhs, env, DoubleType)
      ...
      case JSLinkingInfo() => // Literals

      case _: Literal => // Atomic expressions
      ...
      case VarRef(Ident(name, _)) =>
        env.locals.get(name).fold[Unit] {
          ...
          if (tree.tpe != localDef.tpe)
            reportError(
                s"Variable $name of type ${localDef.tpe} " + s"typed as ${tree.tpe}")
        }
      ...
      case Closure(captureParams, params, body, captureValues) =>
        if (captureParams.size != captureValues.size)
          reportError(
              "Mismatched size for captures: " + s"${captureParams.size} params vs ${captureValues.size} values")
  ...
  private object Env {
    ...
    def fromSignature(thisType: Type,
      ...
      new Env(thisType,
              ...
              Map(None ->
              (if (resultType == NoType) AnyType
                  else resultType)),
              isConstructor)
    }
  }

  private class CheckedClass(val name: String,
                             val kind: ClassKind,
  ...
  private class CheckedField(
      val name: String, val tpe: Type, val mutable: Boolean)
}
