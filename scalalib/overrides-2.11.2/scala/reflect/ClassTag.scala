...
trait ClassTag[T] extends ClassManifestDeprecatedApis[T] with Equals
    with Serializable {
  ...
  private [ this] def unapplyImpl(
    ...
    val conforms =
      runtimeClass.isInstance(x) || (alternative != null &&
          runtimeClass.isAssignableFrom(alternative))
    if (conforms) Some(x.asInstanceOf[T])
    else None
  }
