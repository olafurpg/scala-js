...
class Range(val start: Int, val end: Int, val step: Int)
  ...
  override final val isEmpty =
    ((start > end && step > 0) || (start < end && step < 0) || (start == end &&
        !isInclusive))
  ...
  private def isWithinBoundaries(elem: Int) =
    !isEmpty && ((step > 0 && start <= elem && elem <= last) || (step < 0 &&
        last <= elem && elem <= start))
  ...
  override def equals(other: Any) =
    other match {
      case x: Range =>
        (x canEqual this) && (length == x.length) && (isEmpty ||
...
object Range {
  ...
  private def fail(start: Int, end: Int, step: Int, isInclusive: Boolean) =
    throw new IllegalArgumentException(description(
        ...
        step,
        isInclusive) + ": seqs cannot contain more than Int.MaxValue elements.")
