...
final class JUnitTask(val taskDef: TaskDef, runner: JUnitBaseRunner)
  ...
  def execute(
    ...
    Try(TestUtils.loadModule(bootstrapperName, runner.testClassLoader)) match {
      ...
      case Success(_) =>
        richLogger.error(
            "Error while loading test class: " + taskDef.fullyQualifiedName + ", expected " + bootstrapperName + " to extend JUnitTestBootstrapper")
    ...
    if (runner.runSettings.verbose) {
      ...
      val msg = Seq(c("Test run finished:", INFO),
                    c(s"$failed failed,",
                      if (failed == 0) INFO
                      else ERRCOUNT),
                    c(s"$ignored ignored,",
