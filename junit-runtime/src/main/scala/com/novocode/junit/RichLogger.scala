...
final class RichLogger private(loggers: Array[Logger], settings: RunSettings) {
  ...
  private def logStackTracePart(trace: Array[StackTraceElement],
    ...
    for (i <- top to m2) {
      error("    at " + stackTraceElementToString(
  ...
  private def stackTraceElementToString(e: StackTraceElement,
    ...
    val highlight =
      settings.color && {
        testClassName == e.getClassName || (testFileName != null &&
            testFileName == e.getFileName)
      }
    var r = ""
