...
object Scalajsld {
  ...
  private implicit object OutputModeRead extends scopt.Read[OutputMode] {
    val arity = 1
    val reads = { (s: String) =>
      OutputMode.All.find(_.toString() == s)
  ...
  def main(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[Options]("scalajsld") {
      ...
      arg[File]("<value> ...").unbounded().action { (x, c) =>
        c.copy(cp = c.cp :+ x)
      }.text("Entries of Scala.js classpath to link")
      opt[File]('o', "output").valueName("<file>").required().action {
        (x, c) =>
          c.copy(output = x)
      }.text("Output file of linker (required)")
      opt[File]("jsoutput").hidden().valueName("<file>").abbr("jo").action {
        (_, c) =>
      ...
      opt[File]('r', "relativizeSourceMap").valueName("<path>").action {
        (x, c) =>
      ...
      opt[File]("stdlib").valueName("<scala.js stdlib jar>").hidden().action {
        (x, c) =>
          c.copy(stdLib = Some(x))
      }.text(
          "Location of Scala.js standard libarary. This is set by the " + "runner script and automatically prepended to the classpath. " + "Use -n to not include it.")
      ...
      opt[Unit]("really-quiet").abbr("qq").action { (_, c) =>
    ...
    for (options <- parser.parse(args, Options())) {
      ...
      if (options.jsoutput) {
        Console.err.println(
            "Support for the --jsoutput flag has been dropped. " + "JS dependencies will not be written to disk. " + "Comment on https://github.com/scala-js/scala-js/issues/2163 " + "if you rely on this feature.")
      }

      // Warn if bypassing linking errors was requested.
      if (options.bypassLinkingErrors) {
        Console.err.println(
            "Support for bypassing linking errors with -b or " + "--bypassLinkingErrors will be dropped in the next major version.")
      }
      ...
      val backendConfig = LinkerBackend.Config().withRelativizeSourceMapBase(
          options.relativizeSourceMap).withPrettyPrint(options.prettyPrint)
