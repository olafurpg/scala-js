...
object Bits {
  ...
  private val highOffset =
    if (areTypedArraysBigEndian) 0
    else 1
  private val lowOffset =
    if (areTypedArraysBigEndian) 1
    else 0
  ...
  @inline
  private def encodeIEEE754(ebits: Int, fbits: Int, v: Double): (Boolean, Int,
      Double) = {
    import Math._
