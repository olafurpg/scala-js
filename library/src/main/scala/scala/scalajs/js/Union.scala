...
object | {
  ...
  abstract sealed class EvidenceLowestPrioImplicits {
    ...
    implicit def right[A, B1, B2](implicit ev: Evidence[A, B2]): Evidence[A,
        B1 | B2] = ReusableEvidence.asInstanceOf[Evidence[A, B1 | B2]]
  }
  ...
  abstract sealed class EvidenceLowPrioImplicits
    ...
    implicit def left[A, B1, B2](implicit ev: Evidence[A, B1]): Evidence[A,
        B1 | B2] = ReusableEvidence.asInstanceOf[Evidence[A, B1 | B2]]

    /** If `A <: B`, then `A <: js.UndefOr[B]`. */
    implicit def undefOr[A, B](implicit ev: Evidence[A, B]): Evidence[A,
        UndefOr[B]] = ReusableEvidence.asInstanceOf[Evidence[A, UndefOr[B]]]
  }
  ...
  object Evidence extends EvidenceLowPrioImplicits {
    ...
    implicit def allSubtypes[A1, A2, B](
        ...
        B] = ReusableEvidence.asInstanceOf[Evidence[A1 | A2, B]]
  }
  ...
  implicit class UnionOps[A <: _ | _](val self: A)
      extends AnyVal {
