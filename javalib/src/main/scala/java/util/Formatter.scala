...
final class Formatter(private val dest: Appendable)
  ...
  def format(format_in: String, args: Array[AnyRef]): Formatter =
    ifNotClosed {
        ...
        fmt match {
          ...
          case FormattedChunk(matchResult) =>
            ...
            (conversion: @switch) match {
              ...
              case 's' | 'S' =>
                arg match {
                  case formattable: Formattable =>
                    val flags = {
                      (if (hasFlag("-")) FormattableFlags.LEFT_JUSTIFY
                       else 0) |
                      (if (hasFlag("#")) FormattableFlags.ALTERNATE
                          else 0) |
              ...
              case 'e' | 'E' =>
                sciNotation(if (hasPrecision) precision
                else 6)
              case 'g' | 'G' =>
              ...
              case 'f' =>
                with_+({
                  ...
                  numberArg.toFixed(if (hasPrecision) precision
                  else 6)
            ...
            def sciNotation(precision: Int) = {
              ...
              with_+({
                ...
                if ('e' == exp.charAt(exp.length - 3)) {
                  exp.substring(0, exp.length - 1) + "0" + exp.charAt(
                      exp.length - 1)
                } else exp
              }, numberArg.isNaN || numberArg.isInfinite)
            }
