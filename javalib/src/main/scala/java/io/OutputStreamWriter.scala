...
class OutputStreamWriter(private [ this] var out: OutputStream,
  ...
  def this (out: OutputStream, cs: Charset) = this(
      out,
    ...
    if (!closed) {
      ...
      def loopEncode(): Unit = {
        ...
        if (result.isUnderflow) {
          assert(
              !cbuf.hasRemaining,
              "CharsetEncoder.encode() should not have returned UNDERFLOW when " + "both endOfInput and inBuf.hasRemaining are true. It should have " + "returned a MalformedInput error instead.")
        } else if (result.isOverflow) {
          makeRoomInOutBuf()
          loopEncode()
        } else {
