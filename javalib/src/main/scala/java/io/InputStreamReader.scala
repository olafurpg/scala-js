...
class InputStreamReader(private [ this] var in: InputStream,
  ...
  def this (in: InputStream, charset: Charset) = this(
      in,
  ...
  def this (in: InputStream, charsetName: String) = this(
      in, Charset.forName(charsetName))
  ...
  private def readImpl(out: CharBuffer): Int = {
    ...
    if (out.position != initPos) {
      ...
      if (endOfInput) {
        assert(
            !inBuf.hasRemaining,
            "CharsetDecoder.decode() should not have returned UNDERFLOW when " + "both endOfInput and inBuf.hasRemaining are true. It should have " + "returned a MalformedInput error instead.")
        // Flush
        ...
        if (inBuf.limit == inBuf.capacity) {
          ...
          if (!inBuf.hasRemaining) {
            throw new AssertionError(
                "Scala.js implementation restriction: " + inBuf.capacity + " bytes do not seem to be enough for " + getEncoding + " to decode a single code point. " + "Please report this as a bug.")
          }
