...
final class URI(origStr: String) extends Serializable with Comparable[URI] {
  ...
  def this (scheme: String, ssp: String, fragment: String) = this(
      URI.uriStr(scheme, ssp, fragment))
  ...
  def this (scheme: String, host: String, path: String,
  fragment: String) = this(scheme, null, host, -1, path, null, fragment)
...
object URI {
  ...
  private val uriRe = {
    ...
    val net_path = "//(" + authority + ")(" + abs_path + ")?"
    /*2CAPT*/
  ...
  private val quoteStr: js.Function1[String, String] = { (str: String) =>
    ...
    while (buf.hasRemaining) {
      val c = buf.get & 0xff
      res += (if (c <= 0xf) "%0"
              else "%") + Integer.toHexString(c).toUpperCase
    }
